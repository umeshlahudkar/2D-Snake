...
public class SnakeController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.UpArrow) && !yMovement)
        {
            yMovement = true;
            xMovement = false;
            direction = Vector2.up;
        }
        else if (Input.GetKeyDown(KeyCode.DownArrow) && !yMovement)
        {
            yMovement = true;
            xMovement = false;
            direction = Vector2.down;
        }
        else if (Input.GetKeyDown(KeyCode.LeftArrow) && !xMovement)
        {
            yMovement = false;
            xMovement = true;
            direction = Vector2.left;
        }
        else if (Input.GetKeyDown(KeyCode.RightArrow) && !xMovement)
        {
            yMovement = false;
            xMovement = true;
        ...
        if ((massGainerFoodConsumeCount >= PlayerConstant.scoreBoostPowerActivateAt) && !scoreBoostActive)
        {
        ...
        if(currentShieldPower >= PlayerConstant.shieldPowerActivateAt)
        {
            shieldActive = true;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Grow();
            scoreController.ScoreIncrement(massGainerScore, "P1");
        ...
        if (collision.gameObject.CompareTag("MassBurnerFood"))
        {
            ...
            {
                Shrink();
                scoreController.ScoreIncrement(PlayerConstant.massBurnerScore, "P1");
